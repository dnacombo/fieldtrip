...
cfg.linecolor = ft_getopt(cfg,'linecolor', [0.1 0.1 0.1]);
...
chunksout(chunksout(:,1) > chunksout(:,2),:) = []; % remove any row where end is before start
...
chekcsampleinfo = [begsample endsample];
if not(all(sampleinfo == chekcsampleinfo))
  error;
end
